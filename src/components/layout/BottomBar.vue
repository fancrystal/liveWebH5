...
// When screen share ends, revert mode
...
async function handleCameraToggle() {
  if (mediaStore.isCameraOn && !mediaStore.isCameraVisible) {
    mediaStore.toggleCameraVisibility()
    return
  }
...
const mainBtnClass = computed(() => ({
  'btn-primary': streamStore.status !== 'live',
  'btn-danger':  streamStore.status === 'live',
}))
</script>
  ...
  <footer class="bottom-bar no-select">
    <div class="bottom-bar__tools">
      ...
      <div class="device-group">
        <button
          class="bottom-bar__tool-btn"
          :class="{ active: mediaStore.isMicOn }"
          :title="t('mic')"
        ...
        <!-- Dropdown arrow -->
        <button
          class="bottom-bar__device-arrow"
          :class="{ open: openDropdown === 'mic' }"
          :title="t('selectMic')"
        ...
        <div v-if="openDropdown === 'mic'" class="device-dropdown">
          ...
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" class="device-dropdown__check" v-if="d.deviceId === mediaStore.activeAudioDeviceId">
      ...
      <div class="device-group">
        <button
          class="bottom-bar__tool-btn"
          :class="{
            active:      mediaStore.isCameraOn && mediaStore.isCameraVisible,
            'cam-hidden': mediaStore.isCameraOn && !mediaStore.isCameraVisible,
          }"
          :title="t('camera')"
        ...
        <button
          class="bottom-bar__device-arrow"
          :class="{ open: openDropdown === 'camera' }"
        ...
        <div v-if="openDropdown === 'camera'" class="device-dropdown">
          ...
          >
            <svg width="12" height="12" viewBox="0 0 24 24" v-if="d.deviceId === mediaStore.activeVideoDeviceId" class="device-dropdown__check">
              <polyline points="20 6 9 17 4 12" fill="none" stroke="currentColor" stroke-width="2.5"/>
            </svg>
      ...
      <button
        class="bottom-bar__tool-btn"
        :class="{ active: mediaStore.isScreenSharing }"
        :title="t('share')"
      ...
      <button
        class="bottom-bar__tool-btn"
        :class="{ active: wbStore.activeMode === 'whiteboard' }"
        :title="t('whiteboard')"
      ...
      <button
        class="bottom-bar__tool-btn"
        :class="{ active: wbStore.activeMode === 'document' }"
        :title="t('document')"
    ...
    <div class="bottom-bar__right">
      <button class="bottom-bar__settings-btn" title="设置" @click="onOpenSettings()">
      ...
      <button class="bottom-bar__main-btn" :class="mainBtnClass" @click="onMainAction()">
        {{ mainBtnLabel }}
      </button>
    </div>
  </footer>
...
.bottom-bar {
  ...
  &__tool-btn {
    ...
    &.cam-hidden { color: $color-warning; opacity: 0.75; }

    // Inside device-group: flatten the right side
    .device-group & {
      width: 28px;
  ...
  &__device-arrow {
    ...
    &.open  { background: $color-bg-active; color: $color-accent; }
  }
  ...
  &__main-btn {
    ...
    &.btn-primary { background: $color-accent; color: #fff; &:hover { background: $color-accent-hover; } }
    &.btn-danger  { background: $color-danger;  color: #fff; &:hover { background: #dc2626; } }
  }
}

// ─── Device group (button + arrow + dropdown) ──────────────────────────────
.device-group {
  position: relative;
  display: flex;
  align-items: center;

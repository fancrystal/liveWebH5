...
sequenceDiagram
    ...
    participant STUN as STUN 服务器<br/>(miwifi/qq)
    participant WHIP as WHIP 服务器<br/>(ZLMediaKit / SRS)
    ...
    主播端->>WHIP: 9. POST /index/api/whip?app=live&stream=xxx<br/>Content-Type: application/sdp<br/>Body: SDP Offer（含所有 ICE 候选）
    WHIP-->>主播端: 10. 201 Created<br/>Body: SDP Answer（服务端 ICE 候选 + 编解码确认）
...
```mermaid
flowchart TD
    WB["白板 Canvas<br/>(Fabric.js lower+upper canvas)"]
    DOC["文档 Canvas<br/>(PDF.js 渲染)"]
    CAM["摄像头 &lt;video&gt;"]
    SCR["屏幕共享 &lt;video&gt;"]
    CO["连麦视频<br/>(最多9路 &lt;video&gt;)"]
    MIC["麦克风<br/>AudioTrack"]
    ...
    subgraph MIXER["offscreen canvas (1280×720)"]
        direction TB
        TICK["Web Worker tick<br/>@30fps（后台不降速）"]
        DRAW["draw():<br/>①白底 ②白板/文档 ③共享<br/>④连麦格 ⑤摄像头 PiP"]
        TICK --> DRAW
    end

    MIXER -->|captureStream(30)| VT["VideoTrack<br/>contentHint='detail'"]
    MIC -->|直接添加 不经过 canvas| AT["AudioTrack<br/>48kHz Opus"]

    VT --> MS["MediaStream<br/>= VideoTrack + AudioTrack"]
    AT --> MS
...
stateDiagram-v2
    ...
    good --> degraded: 连续2次: 丢包>2% 或 RTT>100ms
    degraded --> good: 连续2次: 丢包<2% 且 RTT<100ms
    degraded --> poor: 连续2次: 丢包>10% 或 RTT>300ms
    poor --> degraded: 连续2次: 丢包<10% 且 RTT<300ms

    good: good\n码率 100% / 全帧率
    degraded: degraded\n码率 60% / 全帧率\nsender.setParameters() 原地调整
    poor: poor\n码率 30% / ≤15fps\nsender.setParameters() 原地调整

    poor --> reconnecting: ICE disconnected > 5s 或 connectionState=failed
    reconnecting --> good: 重连成功 / ICE 自愈
    reconnecting --> [*]: 重连 4 次失败
```
...
flowchart TD
    ...
    B -- 是 --> Z([结束])
    B -- 否 --> C[启动 5s Grace Timer<br/>等待 NAT 重绑 / Wi-Fi 切换自愈]

    C --> D{5s 后 ICE 状态?}
    D -- "connected/completed\n（自愈成功）" --> E[取消 Timer\n恢复 netQuality=good]
    D -- "仍为 disconnected\n（未自愈）" --> F[立即重连\n重置 reconnectCount=0\n无退避惩罚]

    F --> G[new RTCPeerConnection\ncreateOffer → POST WHIP]
    G --> H{重连成功?}
    H -- 是 --> I[推流恢复\ntoast 提示用户]
...
sequenceDiagram
    participant 主播端 as 主播端 (浏览器)
    participant RELAY as rtmp-relay<br/>(Node.js 中转服务)
    participant FFMPEG as ffmpeg 进程<br/>(relay 内部启动)
    participant RTMP as RTMP 服务器<br/>(SRS / 云直播)

    主播端->>主播端: 1. new MediaRecorder(MediaStream)<br/>mimeType: video/webm;codecs=h264,opus<br/>videoBitsPerSecond: 配置码率

    主播端->>RELAY: 2. WebSocket 握手<br/>ws(s)://host/rtmp-relay?rtmp=推流地址

    RELAY->>FFMPEG: 3. spawn ffmpeg<br/>-i pipe:0 -c:v copy -c:a aac -f flv rtmp://...
    FFMPEG->>RTMP: 4. 建立 RTMP 连接（TCP 1935）

    主播端->>主播端: 5. recorder.start(100)<br/>每 100ms 触发 ondataavailable

    loop 每 100ms
        主播端->>主播端: 检查 socket.bufferedAmount
        alt bufferedAmount < 2MB（正常）
            主播端->>RELAY: 发送 WebM chunk（二进制帧）
            RELAY->>FFMPEG: pipe.stdin.write(chunk)
            FFMPEG->>RTMP: 推送 FLV 数据
        else bufferedAmount > 2MB（拥塞）
            主播端->>主播端: 丢弃本帧（实时性优先）
            主播端->>主播端: 持续 8s → recovery reconnect
        end
    end

    Note over 主播端,RELAY: 每 30 分钟：planned restart
...
stateDiagram-v2
    ...
    degraded: degraded\n64KB ~ 2MB\n发送但警告用户
    poor: poor\n> 2MB\n丢弃当前帧
    recovery: 恢复重连\nbufferedAmount > 2MB 持续 8s

    good --> degraded: bufferedAmount > 512KB
    degraded --> good: bufferedAmount < 64KB
    degraded --> poor: bufferedAmount > 2MB
    poor --> degraded: bufferedAmount < 2MB
    poor --> recovery: 持续 > 8s 且距上次重连 > 60s

    recovery --> good: 重连成功 新 ffmpeg 进程\n清空 TCP 积压缓冲
    recovery --> [*]: 重连 4 次失败

    good --> reconnecting: WS 意外断开
    reconnecting --> good: 指数退避重连成功\n3→6→12→24s
    reconnecting --> [*]: 重连 4 次失败
```
...
sequenceDiagram
    ...
    REC->>WS: 发送新 WebM EBML 头（同一 WS）
    WS->>RELAY: 旧 ffmpeg 收到 WebM 中途插入新头部
    RELAY->>FFMPEG: ❌ Opus packet header error → ffmpeg 崩溃
...
| 兼容性 | 现代浏览器均支持 | Chrome / Edge（Safari 不支持 MediaRecorder h264） |
...
```mermaid
flowchart LR
    subgraph 卡点E["卡点 E：K8s nginx 缓冲导致卡顿"]
    ...
    subgraph 卡点F["卡点 F：30分钟 Opus 崩溃"]
        ...
        F2 --> F3["✅ 已修复：close WS → 新 WS\n新 ffmpeg 进程接收干净 WebM"]
    end

    subgraph 卡点G["卡点 G：TCP 积压导致延迟飙升"]
        G1["弱网 socket.bufferedAmount 堆积\n观众收到'过去内容'"]
        G1 --> G2["✅ 已修复：bufferedAmount > 2MB\n丢弃当前帧\n持续 8s 触发 recovery 重连"]
    end

    subgraph 卡点H["卡点 H：Safari 不支持"]
        H1["Safari MediaRecorder\n不支持 h264/vp8 WebM"]
        H1 --> H2["⚠️ 现状：Safari 降级提示\n切换 WHIP 模式\n或接入 Safari 专用 RTMP SDK"]
    end
```
...
**现状**：`roomStore.room.viewerCount` 字段已存在，但数据来源尚未对接。
...
sequenceDiagram
    participant 观众端
    participant 信令服务 as 信令服务器<br/>(socket.io)
    participant 主播端

    观众端->>信令服务: 加入房间 join-room { roomId }
    信令服务->>信令服务: 维护房间人数计数器
    ...
    信令服务->>信令服务: 计数器 -1（含 30s 去抖，防刷新抖动）
    信令服务->>主播端: viewer-count-update { count: 127 }
```

**待开发工作：**
- 信令服务端：`join-room` / `leave-room` 事件处理 + 房间人数维护
- 心跳机制：观众端每 30s 发 `heartbeat`，服务端超时 60s 自动踢出
- 主播端：在 `SignalService.on('viewer-count-update')` 中更新 store
...
**现状**：`roomStore.room.watchUrl` 字段已存在，TopBar 组件有占位显示，未实现复制/分享功能。
...
**待开发工作：**
- 根据 `streamStore.config.mode` 和推流地址规则自动生成拉流 URL
- TopBar 添加「复制链接」「生成二维码」按钮（qrcode.js）
- 支持多协议地址切换展示（WebRTC / HLS / FLV）
...
**现状**：`chatStore` 消息模型完整，UI 组件 `RightPanel` 已有聊天面板骨架，**信令收发尚未对接**。
...
sequenceDiagram
    ...
    主播端->>信令服务: chat-pin { messageId } ← 置顶消息
    信令服务->>观众端: chat-pinned { messageId }（广播）

    主播端->>信令服务: chat-ban { userId } ← 禁言
    信令服务->>观众端: 该用户后续消息被拦截
```
...
| 功能 | 优先级 | 工作量 |
|------|--------|--------|
| 消息收发对接（`SignalService.on('chat-message')`） | P0 | 0.5 天 |
...
**现状**：`useCoStream.ts` WebRTC P2P 框架已完整实现，信令事件处理齐全，**缺少服务端信令转发实现和完整 UI 流程**。
...
sequenceDiagram
    ...
    participant 信令服务 as 信令服务器<br/>(socket.io)
    participant 主播端
    participant 合流器 as StreamMixer<br/>(offscreen canvas)

    观众端->>信令服务: co-apply { userId, nickname } ← 申请连麦
    信令服务->>主播端: co-apply { userId, nickname }
    主播端->>主播端: coStreamStore.addApply()\nUI 弹出申请通知

    主播端->>信令服务: accept-apply { userId } ← 主播同意
    信令服务->>观众端: co-accepted ← 通知观众

    观众端->>观众端: getUserMedia() 获取摄像头+麦克风
    主播端->>主播端: callPeer(userId)\ncreateOffer()
    ...
    主播端->>主播端: ontrack → remoteStream 到 coStreamStore
    主播端->>合流器: 连麦视频进入宫格区域合流推出
...
flowchart TD
    subgraph 已完成["✅ 已完成"]
        ...
        B["coStreamStore\n参与者状态管理"]
        C["StreamMixer\n连麦视频合流进推流画面"]
        D["申请/同意/踢出逻辑\ncallPeer/answerOffer/kickParticipant"]
    end
...
| useCoStream ICE 服务器换国内 STUN | P1 | 0.5 天 |
| 观众端连麦页面 | P1 | 3 天 |
| TURN 服务器接入（对称 NAT 穿透） | P2 | 1 天 |
...
| 问题 | 根因 | 解决方案 | 状态 |
|------|------|---------|------|
| 摄像头 PiP 占满半个画面 | PiP 用白板 canvas 像素计算，UI 收窄时比例失控 | 改为百分比坐标系，与容器尺寸解耦 | ✅ |
...
| WHIP 持续重连 | Google STUN 国内不可达，无 srflx 候选 | 切换 miwifi/qq STUN | ✅ |
| Chrome 自动降分辨率 | Chrome BWE 弱网时降分辨率 | `scaleResolutionDownBy: 1.0` | ✅ |
| 本地开发 RTMP 无法连接 | Vite 没有 `/rtmp-relay` 代理 | Vite proxy 配置 WebSocket 转发 | ✅ |
